...
export default function StudentAttendance() {
    ...
    const courseBreakdown: CourseBreakdownItem[] = Object.entries(courseStats).map(([name, stats], index) => ({
        name,
    ...
    const getDayStatus = (date: Date): 'present' | 'absent' | 'no-class' | null => {
        ...
        const dayRecords = attendanceRecords.filter(r => 
            r.session?.start_time && isSameDay(new Date(r.session.start_time), date)
        );
                                        ...
                                        <tr key={i} className="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-sm text-gray-500">
                                                {record.session?.start_time && record.session?.end_time
                                                    ? `${(new Date(record.session.end_time).getTime() - new Date(record.session.start_time).getTime()) / (1000 * 60 * 60)}h`
                                                    : '1h'}
                                            </td>
