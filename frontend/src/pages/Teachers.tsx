...
interface Teacher {
    ...
    phone?: string;
    role: string;
    status: boolean | string; // API might return boolean or string 'active'
    created_at: string;
}
...
export default function Teachers() {
    ...
    const fetchTeachers = async () => {
        try {
            ...
            const teacherList = Array.isArray(allUsers) 
                ? allUsers.filter((u: any) => u.role?.toLowerCase() === 'teacher').map((u: any) => ({
                    ...u,
                    status: u.is_active !== undefined ? u.is_active : (u.status === 'active' || u.status === true)
                })) 
                : [];
    ...
    const handleManageCourses = async (teacher: Teacher) => {
        setSelectedTeacher(teacher);
        setIsAssignModalOpen(true);
        fetchTeacherCourses(teacher.id);
    };

    const fetchTeacherCourses = async (teacherId: number | string) => {
        try {
            const data = await getTeacherCourses(teacherId);
            setAssignedCourses(Array.isArray(data) ? data : []);
        } catch (error) {
    ...
    const handleStatusToggle = async (teacher: Teacher) => {
        try {
            ...
            // API expects is_active or status? usually snake_case for python backend
            await updateUser(teacher.id, { is_active: newStatus }); 
    ...
    const handleManageTimeSlots = async (teacher: Teacher) => {
        setSelectedTeacher(teacher);
    ...
    const fetchTeacherTimeSlots = async (teacherId: number | string) => {
        try {
            const slots = await getTeacherTimeSlots(teacherId);
            setTeacherSlots(Array.isArray(slots) ? slots : []);
        } catch (error) {
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-slate-500">ID: {String(teacher.id).substring(0, 8)}...</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <div>
                                ...
                                {teacherSlots.length === 0 ? (
                                    ...
                                    <div className="space-y-3 max-h-96 overflow-y-auto">
                                        {/* Group slots by date */}
                                        {Object.entries(
                                            teacherSlots.reduce((acc: Record<string, TimeSlot[]>, slot) => {
