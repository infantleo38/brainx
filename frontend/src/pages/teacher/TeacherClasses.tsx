...
// Assuming mock data is JS or needs valid TS type if imported. 
// If mock data is .ts, it should be fine. If .js, might need allowJs or declaration.
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const myBatches = batches.filter((b: Batch) => {
                ...
                const mappedClasses = myBatches.map((batch: Batch) => {
                    const course = courses.find((c: Course) => String(c.id) === String(batch.course_id));
                    return {
                        id: batch.id,
            ...
            <div className="flex-1 overflow-y-auto p-8 scroll-smooth bg-background-light">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                        {classesStats.map((stat: ClassStat, index: number) => (
